...
// Reedicion por Hector Gomez Para vector z
//-----------------------------------------------------------------------------
...
//			SetButtonEnabled( "PauseButton", _mediaPlayer.m_AutoStart );
        ...
        public void OnOpenVideoFileIndex(int video_int)
        {
            if (GENERAL_INTERFAZ.video_llamado)
            {
                _mediaPlayer.m_VideoPath = string.Empty;
                _mediaPlayer.m_VideoPath = _videoFiles[video_int];
                if (string.IsNullOrEmpty(_mediaPlayer.m_VideoPath))
                {
                    _mediaPlayer.CloseVideo();
                }
