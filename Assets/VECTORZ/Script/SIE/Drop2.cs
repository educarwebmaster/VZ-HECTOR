...
public class Drop2 : MonoBehaviour, IDropHandler
{
    ...
    public bool Check;
    public SELECCION_BIBLIOTECA Controlador;
    ...
    {
        Drag2.item.GetComponent<Drag2>().Restore();//reestaurar pocicion
    }

    public void InsertDrag()// incertar elemento como hijo
    {
        Drag2.item.transform.SetParent(transform);
        //Drag.item.GetComponent<LayoutElement>().ignoreLayout = false;
        if (item.GetComponent<Drag2>().ID == ID)//comparar id para saber si son correctos
        {
